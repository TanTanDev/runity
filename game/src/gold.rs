...
use crate::{RemoveOnReset};

#[derive(Component)] pub struct YVelocity(pub f32);
#[derive(Component)] pub struct Gold;

pub fn gold_movement_system(
    time: Res<bevy::core::Time>,
    mut query: Query<(Entity, &mut Transform, &mut YVelocity), With<Gold>>)
{
...
pub fn gold_destroy_system(
    mut commands: Commands,
    query: Query<(Entity, &Transform), With<Gold>>)
{
...
pub enum GoldSpawnerState {
    None,
    Spawning(Timer),
}

pub fn gold_spawn_system(
    mut commands: Commands,
    time: Res<bevy::core::Time>,
    mut gold_spawner_state: ResMut<GoldSpawnerState>,
) {
    let spawn_timer: &mut Timer = match &mut *gold_spawner_state {
        GoldSpawnerState::None => return,
        GoldSpawnerState::Spawning(timer) => timer,
    };
